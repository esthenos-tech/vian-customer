...
const columns = [
  {
    ...
    sort: true,
    formatter: (cell, row) => (
      <Link to={`/v2/customer/sme/${cell}`}> {cell} </Link>
    ),
  },
  {
...
class Customers extends React.Component {
  ...
  partnerSubmitHandler = (event) => {
    ...
    axios({
      method: "Post",
      url: `${this.state.baseURL}/api/v2/customers/sme`,
      data: {
  ...
  async componentDidMount() {
    const { data } = await axios.get(
      `${this.state.baseURL}/api/v2/customers/sme`,
      {
        headers: {
    ...
    return (
      <div>
        <Card>
          ...
          <CardHeader>
            <CardTitle>List Of SME Customers</CardTitle>
          </CardHeader>
          <CardBody>
            <BootstrapTable
              bootstrap4
